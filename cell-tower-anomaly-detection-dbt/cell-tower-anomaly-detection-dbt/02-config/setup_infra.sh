...
if [ "${SERVERLESS_SPARK}" = true ] ; then
    ...
    if [ ! "${?}" -eq 0 ];then
      LOG_DATE=`date`
      echo "git not found! Please install first"
      exit 1
    fi
...
if [  "${?}" -eq 0 ]; then
    ...
    LOG_DATE=`date`
    echo "${LOG_DATE} SA does not exist -  Creating Service Account ..  ${TERRAFORM_SA_NAME}"
    "${GCLOUD_BIN}" iam service-accounts create "${TERRAFORM_SA_NAME}"
    if [ ! "${?}" -eq 0 ];then
      LOG_DATE=`date`
      echo "Unable to run ${GCLOUD_BIN} iam service-accounts create ${TERRAFORM_SA_NAME}"
...
TF_SA_ROLES_LIST="roles/bigquery.admin roles/dataproc.admin roles/compute.admin  roles/resourcemanager.projectIamAdmin roles/iam.securityAdmin roles/iam.serviceAccountAdmin roles/iam.serviceAccountUser roles/servicenetworking.networksAdmin roles/storage.admin roles/iam.serviceAccountTokenCreator"
...
TIME_TO_SLEEP=60
echo "#################################################################"
echo "${LOG_DATE} Sleeping ${TIME_TO_SLEEP} seconds to propagate IAM changes ..."
sleep "${TIME_TO_SLEEP}"
LOG_DATE=`date`
echo "${LOG_DATE} Checking if SA key exists ..."
