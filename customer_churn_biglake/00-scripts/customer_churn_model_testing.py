...
biglake_table_name=sys.argv[5]

sparkDF = spark.read \
          .format('bigquery') \
          .load(project_name+'.'+dataset_name+'.'+biglake_table_name)
...
rf_model = PipelineModel.load(os.path.join('gs://'+bucket_name+'/customer_churn_biglake/'+user_name+'_churn_model/model_files'))
...
predic_filter = predic.select("customerID","gender","SeniorCitizen","Partner","Dependents","tenure","PhoneService","MultipleLines","InternetService","OnlineSecurity","OnlineBackup","DeviceProtection","TechSupport","StreamingTV","StreamingMovies","Contract","PaperlessBilling","PaymentMethod","MonthlyCharges","TotalCharges","prediction")

spark.conf.set("parentProject", project_name)
bucket = bucket_name
spark.conf.set("temporaryGcsBucket",bucket)
predic_filter.write.format('bigquery') \
.mode("overwrite")\
