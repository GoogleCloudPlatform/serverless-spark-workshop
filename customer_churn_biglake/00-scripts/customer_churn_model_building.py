...
test_pred_filter = test_pred.select("customerID","gender","SeniorCitizen","Partner","Dependents","tenure","tenure_group","PhoneService","MultipleLines","InternetService","OnlineSecurity","OnlineBackup","DeviceProtection","TechSupport","StreamingTV","StreamingMovies","Contract","PaperlessBilling","PaymentMethod","MonthlyCharges","TotalCharges","Churn","label","prediction")

spark.conf.set("parentProject", project_name)
bucket = bucket_name
spark.conf.set("temporaryGcsBucket",bucket)
test_pred_filter.write.format('bigquery') \
.mode("overwrite")\
.option('table', project_name+':'+dataset_name+'.'+user_name+'_predictions_data') \
.save()

rf_model.write().overwrite().save('gs://'+bucket_name+'/customer_churn_biglake/'+user_name+'_churn_model/model_files')
